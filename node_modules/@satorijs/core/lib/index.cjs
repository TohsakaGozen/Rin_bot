...
// satori/packages/core/src/modulator.ts
...
var Modulator = class {
  ...
  async send(content) {
    ...
    if (await this.session.app.serial(this.session, "before-send", this.session))
      return;
...
__name(Modulator, "Modulator");

// satori/packages/core/src/index.ts
var Context = class extends cordis.Context {
  constructor(options) {
    super(options);
    this._components = /* @__PURE__ */ Object.create(null);
    this.on("internal/warning", (format, ...args) => {
      this.logger("app").warn(format, ...args);
    });
    this.on("before-send", async (session) => {
      session.elements = await import_element3.default.transformAsync(session.elements, this._components, session);
    });
  }
  logger(name) {
