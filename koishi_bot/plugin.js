...
const url='http://m.qpic.cn/psc?/V51QYEn34X1RJ92SjrrY0cjKjO0LNH76/ruAMsa53pVQWN7FLK88i5rN9bpwqnGaI0S6aWwR6Gn.u4ifG0JJFkA8SqDDA6xWMa99DvPDfnNmEJFEROx8b..Kn6u0qSQcIYMJ5PvYlsFI!/b&bo=egWAAlwHXAMBCdc!&rf=viewer_4'
...
const reqUnDoPeople = function (session) {
    login({
...
const sendUndoPeople = async function (undo,session) {
	text=`未作青年大学习的名单为：${undo}    ！      请以上同学加紧时间做哦`
	session.bot.internal.sendGroupMsg('309449161',text).then(()=>{ session.send("青年大学习信息已发送到班群")}).catch((error)=>{session.send("未知原因，发送失败")})
